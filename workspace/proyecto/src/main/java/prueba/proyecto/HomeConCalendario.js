...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  calendar = new FullCalendar.Calendar(calendarEl, {
	...
	customButtons: {
		customSearch: {
	  	text: '', // Sin texto, lo reemplazaremos
	    click: null // No hace falta acción
	  }
	},
  ...
  setTimeout(() => {
    const customButton = document.querySelector('.fc-customSearch-button');
    if (customButton) {
     ...
     `;
      ...
      customButton.style.border = 'none'; // Quita el borde del botón
      customButton.style.background = 'transparent'; // Quita el fondo del botón
      customButton.style.boxShadow = 'none'; // Quita sombra si la hay
      customButton.style.padding = '0'; // Ajusta el padding

      input.style.border = '1px solid #ccc'; // Opcional: define borde del input
      input.style.outline = 'none'; // Quita el borde azul al enfocar
      customButton.querySelector('div').appendChild(input);
...
function applyFilters() {
  const searchText = document.getElementById('searchTask').value.toLowerCase().trim();
  ...
  calendar.getEvents().forEach(event => {
    ...
    const priorityMatch = !selectedPriority || event.extendedProps.prioridad === selectedPriority;
    const statusMatch = !selectedStatus || event.extendedProps.status === selectedStatus;

    const showEvent = titleMatch && priorityMatch && statusMatch;
